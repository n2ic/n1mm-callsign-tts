...
def to_nato(call):
    """Convert callsign into NATO phonetics."""
...
def digits_to_words(num_str):
    """Convert each digit into its spoken form with extra space for clarity."""
    return '  '.join(phonetic_map[d] for d in num_str if d.isdigit())  # double space between digits
        ...
        try:
            ...
            if sntnr_val:
                ...
                if formatted_nr and formatted_nr != last_sntnr:
                    spoken_digits = digits_to_words(formatted_nr)
                    report_text = f"you are five nine {spoken_digits}"
                    print(f"[RX] Sent number: {sntnr_val} → formatted: {formatted_nr} → spoken: {spoken_digits}")
                    generate_tts(report_text, N1MM_WAV_PATH_REPORT)
                    generate_tts(spoken_digits, N1MM_WAV_PATH_NR)
